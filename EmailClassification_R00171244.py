# -*- coding: utf-8 -*-
"""
Created on Sun Mar 17 12:52:49 2019
...
def readHamMails():    
    path_ham="D:/Anjukutty/CIT/Semester2/MachineLearning/Assignment1/enron1_smallest/ham/*.txt"  
...
def readSpamMails():
    path_spam = "D:/Anjukutty/CIT/Semester2/MachineLearning/Assignment1/enron1_smallest/spam/*.txt"
...
def buildModel5(train_set,test_set): 
    ...
    ('features', FeatureUnion([
        ...
        ('length', Pipeline([
            ('count', FunctionTransformer(getMailLength, validate=False)),
        ]))
    ])),
    ('clf', SVC(kernel='linear'))])   
    clf_pipe.fit(train_set.emails, train_set.target)    
...
def visualiseModelAccuracy(accuracyDF):
    ...
    ax1.set_title("Model Comaprison Based on F1Score")
    plt.show()
  
    ax2= accuracyDF['AUC'].plot(kind='bar')
    ax2.set_ylabel("Model AUC")  
    ax2.set_xticklabels(accuracyDF['ModelName'])
    ax2.set_title("Model Comaprison Based on Area Under Curve")
    plt.show()
